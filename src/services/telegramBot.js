...
if (!token) {
    ...
    bot.on('message', async (msg) => {
        // تجاهل الرسائل من البوت نفسه أو التي ليست رداً
        if ((botId && msg.from.id === botId) || !msg.reply_to_message) {
            return;
        }
        ...
        try {
            ...
            if (!originalMessageText) {
                return console.warn("Reply to a message with no text/caption. Ignoring.");
            }

            const match = originalMessageText.match(/ID:\s*(\w{24})/);
            if (!match || !match[1]) {
                return bot.sendMessage(msg.chat.id, "لم أتمكن من العثور على معرف المستخدم. تأكد من أنك ترد على الرسالة الصحيحة.");
            }
            ...
            if (!chat) {
                return bot.sendMessage(msg.chat.id, `لا توجد محادثة نشطة للمستخدم بالمعرف: ${userId}`);
            }
        ...
        try {
            if (type === 'dispute') {
                ...
                if (action === 'approve') {
                    ...
                    const io = getIo();
                    io.to(order.user.toString()).emit('new-notification', { userId: order.user.toString(), notification });
                    io.to(order.user.toString()).emit('dispute-resolved', order);
            ...
            if (type === 'reason') {
                ...
                let reasonText = '';
                if (action === 'private') reasonText = 'الحساب كان خاصاً وقت التحقق.';
                else if (action === 'link') reasonText = 'الرابط المقدم غير صحيح أو لا يعمل.';
                else reasonText = 'الخصم صحيح حسب سياسة الخدمة.';
                ...
                const io = getIo();
                io.to(order.user.id.toString()).emit('new-notification', { userId: order.user.id.toString(), notification });
                io.to(order.user.id.toString()).emit('dispute-resolved', order);
