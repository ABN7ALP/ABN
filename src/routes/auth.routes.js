...
router.post('/forgot-password', async (req, res) => {
    try {
        ...
        // لأمان إضافي، لا نخبر المستخدم إذا كان البريد غير موجود
        if (!user) {
            return res.json({ 
        ...
        // إنشاء توكن فريد لإعادة التعيين
        const resetToken = jwt.sign(
            { id: user._id }, 
            process.env.JWT_SECRET + user.password, // استخدام كلمة المرور في التوكن لأمان إضافي
        ...
        // 🔐 هنا يجب إرسال الإيميل فعلياً
        // حالياً سنطبع الرابط في الكونسول للاختبار
        const resetUrl = `${req.protocol}://${req.get('host')}/reset-password.html?token=${resetToken}`;
        
        console.log('📧 رابط إعادة تعيين كلمة المرور:');
        console.log(`للبريد: ${email}`);
        console.log(`الرابط: ${resetUrl}`);
        
        // في البيئة الحقيقية، استخدم خدمة مثل SendGrid أو Nodemailer هنا
        // await sendPasswordResetEmail(email, resetUrl);

        res.json({ 
            message: 'تم إرسال رابط إعادة تعيين كلمة المرور إلى بريدك الإلكتروني',
            // للاختبار فقط - إزالة هذا في البيئة الإنتاجية
            debugUrl: resetUrl 
        });
...
router.post('/reset-password', async (req, res) => {
    try {
        ...
        // البحث عن المستخدم بالتوكن الصالح
        const user = await User.findOne({
        ...
        if (!user) {
            return res.status(400).json({ message: 'رابط إعادة التعيين غير صالح أو منتهي' });
        }
