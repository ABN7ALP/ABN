...
// ... (دوال getRegisterPage و registerUser تبقى كما هي) ...
...
authController.registerUser = async (req, res) => {
    ...
    let errors = [];
    if (!name || !email || !password || !password2) { errors.push({ msg: 'الرجاء ملء جميع الحقول' }); }
    if (password !== password2) { errors.push({ msg: 'كلمتا المرور غير متطابقتين' }); }
    if (password.length < 6) { errors.push({ msg: 'يجب أن تكون كلمة المرور 6 أحرف على الأقل' }); }
...
authController.loginUser = async (req, res) => {
    ...
    try {
        ...
        // ======================= الإصلاح الحاسم هنا =======================
        // 2. قم بحفظ الجلسة يدوياً قبل إعادة التوجيه
        req.session.save(err => {
            if (err) {
                // إذا حدث خطأ في حفظ الجلسة، اطبعه وأرسل المستخدم إلى صفحة الدخول
                console.error("Session save error:", err);
                return res.redirect('/auth/login');
            }

            // 3. بعد الحفظ الناجح، قم بإعادة التوجيه
            if (user.role === 'admin') {
